...
# Output paths and folder
SNR = 0
MODELS_OUTPUT_NAME = f"epileptics_simulated_SNR_{SNR}_and_healthy_simulated"
MODELS_OUTPUT_PATH = Path("models") / MODELS_OUTPUT_NAME
PLOTS_OUTPUT_FOLDER = (
    Path("diplomka") / "plots2" / f"epileptics_simulated_SNR_{SNR}_and_healthy_simulated"
)
...
cv_splits = NNDiplomkaFunctions.create_cross_validation_splits_with_constraints(
    patients, n_splits=10
)
...
# Plot locations 
all_epileptic_patient_ids = []
all_epileptic_accuracies_cv_plot = []
...
for fold_idx, (train_patients, test_patients) in enumerate(cv_splits):
    ...
    epileptics_simulated = NNControl.get_patient_ids_by_type(
        patient_types, PatientType.EPILEPTIC_SIMULATED
    )
    healthy_simulated = NNControl.get_patient_ids_by_type(
        patient_types, PatientType.HEALTHY_SIMULATED
    )

    epileptic_ids = NNControl.get_patient_ids_by_type(
        {patient.id: patient.patient_type for patient in test_patients}, PatientType.EPILEPTIC_SIMULATED
    )
    ...
    fold_sensitivity_stats = (
        NNEvaluationMetrics.compute_sensitivity_stats_for_epileptic(
            patient_metrics, epileptics_simulated
        )
    )
    ...
    fold_specificity_stats = NNEvaluationMetrics.compute_specificity_stats_for_healthy(
        patient_metrics, healthy_simulated
    )
    ...
    epileptic_accuracies_cv_plot = [
    NNEvaluationMetrics.compute_accuracy_for_patient_group(patient_metrics, [pid])
    for pid in epileptics_simulated
]

    # Append the list of accuracies for this fold
    all_epileptic_accuracies_cv_plot.append(epileptic_accuracies_cv_plot)
    ...
    all_epileptic_patient_ids.append(epileptic_ids)

    current_snr_metrics.extend(
        [
            {
                "SNR": SNR,
                "Metric": "Sensitivity",
                "Value": fold_sensitivity_stats["mean"],
            },
            {
                "SNR": SNR,
                "Metric": "Specificity",
                "Value": fold_specificity_stats["mean"],
            },
        ]
    )
...
logger.info("Overall AUC Across All Folds:")
...
with open(PLOTS_OUTPUT_FOLDER / f"all_test_labels_folds_SNR_{SNR}.pkl", "wb") as f:
    pickle.dump(all_test_labels_folds, f)

with open(PLOTS_OUTPUT_FOLDER / f"all_test_probs_folds_SNR_{SNR}.pkl", "wb") as f:
    pickle.dump(all_test_probs_folds, f)

with open(PLOTS_OUTPUT_FOLDER / f"n_folds_SNR_{SNR}.pkl", "wb") as f:
...
NNPlots.plot_mean_roc_curve(
    ...
    save_path=PLOTS_OUTPUT_FOLDER / f"roc_curve_SNR_{SNR}.pdf",
    save_roc_data_path=PLOTS_OUTPUT_FOLDER / f"roc_curve_data_SNR_{SNR}",
)

with open(PLOTS_OUTPUT_FOLDER / f"all_epileptic_accuracies_SNR_{SNR}.pkl", "wb") as f:
    pickle.dump(all_epileptic_accuracies, f)

with open(PLOTS_OUTPUT_FOLDER / f"all_healthy_accuracies_SNR_{SNR}.pkl", "wb") as f:
...
NNPlots.plot_accuracy_boxplot(
    ...
    save_path=PLOTS_OUTPUT_FOLDER / f"accuracy_boxplot_SNR_{SNR}.pdf",
    save_data_path=PLOTS_OUTPUT_FOLDER / f"accuracy_data_SNR_{SNR}",
    snr=SNR,
)

with open(PLOTS_OUTPUT_FOLDER / f"all_sensitivity_stats_SNR_{SNR}.pkl", "wb") as f:
    pickle.dump(all_sensitivity_stats, f)

with open(PLOTS_OUTPUT_FOLDER / f"all_specificity_stats_SNR_{SNR}.pkl", "wb") as f:
    pickle.dump(all_specificity_stats, f)

with open(PLOTS_OUTPUT_FOLDER / f"all_fnr_stats_SNR_{SNR}.pkl", "wb") as f:
    pickle.dump(all_fnr_stats, f)

with open(PLOTS_OUTPUT_FOLDER / f"all_fpr_stats_SNR_{SNR}.pkl", "wb") as f:
...
NNPlots.plot_combined_metrics_boxplot(
    ...
    save_path=PLOTS_OUTPUT_FOLDER / f"combined_metrics_boxplots_SNR{SNR}.pdf",
    save_data_path=PLOTS_OUTPUT_FOLDER / f"combined_metrics_data_SNR{SNR}",
    snr=SNR,
)


with open(PLOTS_OUTPUT_FOLDER / f"all_epileptic_patient_ids_SNR_{SNR}.pkl", "wb") as f:
    pickle.dump(all_epileptic_patient_ids, f)

with open(PLOTS_OUTPUT_FOLDER / f"all_epileptic_accuracies_cv_plot_SNR_{SNR}.pkl", "wb") as f:
    pickle.dump(all_epileptic_accuracies_cv_plot, f)

NNPlots.plot_accuracy_by_location_across_snr(
    patient_ids_folds=all_epileptic_patient_ids,
    accuracies_folds=all_epileptic_accuracies_cv_plot,
    save_path=PLOTS_OUTPUT_FOLDER / f"epileptic_accuracy_by_location_cv_SNR_{SNR}.pdf"
)